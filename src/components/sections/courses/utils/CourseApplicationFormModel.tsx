...
import { RequestFormData } from "./CourseAdmissionFormSection";
...
import { CourseCard, CourseItem } from "@/components/layouts/utils/types";
...
const CourseApplicationFormModel = ({
    ...
    const onSubmit = async (payload: RequestFormData) => {
        ...
        const admissionPayload = {
            ...
            Course: selectedCourse?.course_list?.documentId ?? selectedCourseItem?.documentId,
            step_0: true,
        };
        ...
        try {
            if (isAdmissionOpen?.data?.isAdmission) {
                ...
                if (isExistingEmail?.exists) {
                    setError("Email", { message: `This email ID is already registered for the ${selectedCourse?.course_list?.Name ?? selectedCourseItem?.Name}. A continuation link has already been shared via email. Please use that link to continue the registration or enter a new email ID to start a new registration.` });
                    toast.error(
