...
import type { PaymentData, PaymentResponse } from "@/components/sections/admission-form/_steps/preview-form"
...
const PaymentModel = ({ isOpen, onClose, amount, gstRate, total, paymentDetails }: PaymentPopupProps) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent aria-describedby={undefined} className="max-w-2xl border-0 bg-white p-0 shadow-xl [&>button]:hidden rounded-none">
                <DialogTitle className="hidden">Proceed to Pay</DialogTitle>
                <div className="relative p-8">
                    <div className="mb-6 flex items-start justify-between">
                        <h2 className="3xl:text-2xl font-semibold text-black">Proceed to Pay</h2>
                        <button
                            onClick={onClose}
                    ...
                    <div className="mb-8">
                        <p className="3xl:text-lg text-gray-600">
                            Proceed to pay <span className="font-medium text-[#E87A6C]">₹{amount.toLocaleString("en-IN")}</span> +{" "}
                            <span className="font-medium text-[#E87A6C]">{gstRate}% GST</span>{" "}
                            <span className="font-medium text-gray-700">
                                (Total: <span className="text-[#E87A6C]">₹{total.toLocaleString("en-IN")}</span>)
                            </span>{" "}
                            to continue.
                        </p>
                    </div>
