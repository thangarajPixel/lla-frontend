...
const GallertSection = () => {
  const galleryImages = Array(8).fill(Gallery);
  ...
  return (
    <section className="w-full py-8 md:py-12 lg:py-16 xl:py-20 2xl:py-24 3xl:py-28 max-w-[1920px] mx-auto lg:w-[90vw]">
      ...
      <div className="hidden lg:grid grid-cols-5 gap-9 2xl:gap-6">
        <ImageWidget src={Gallery} alt="Gallery" className="w-full max-h-[250px] object-cover" />
        <div />
        ...
        <ImageWidget src={Gallery} alt="Gallery" className="w-full max-h-[250px] object-cover" />
        <div>
          <ImageWidget src={Gallery} alt="Gallery" className="w-full min-h-[350px] mt-20 max-h-[250px] object-cover" />
        </div>
        <ImageWidget src={Gallery} alt="Gallery" className="w-full max-h-[250px] object-cover" />
        <div />
      </div>
      ...
      <div className="lg:hidden space-y-4 overflow-hidden">
        <div className="relative overflow-hidden">
          <div className="flex animate-marquee-left gap-4">
            {[...galleryImages, ...galleryImages].map((img, idx) => (
              <div key={`row1-${idx}`} className="relative shrink-0 w-[45vw] aspect-square overflow-hidden">
                <ImageWidget src={img} alt="Gallery" fill className="object-cover" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative overflow-hidden">
          <div className="flex animate-marquee-right gap-4">
            {[...galleryImages, ...galleryImages].map((img, idx) => (
              <div key={`row2-${idx}`} className="relative shrink-0 w-[45vw] aspect-square overflow-hidden">
                <ImageWidget src={img} alt="Gallery" fill className="object-cover" />
              </div>
            ))}
          </div>
