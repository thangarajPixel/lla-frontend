...
import { getBlogPageData, getMediaPageData } from "@/app/api/server";
...
const MediaSection = ({ data }: { data: MediaPageData }) => {
  ...
  const [mediaCards, setMediaCards] = useState(mediaData?.MediaCard || []);
  ...
  const total =
    searchTotal !== null
      ? searchTotal
      : data?.pagination?.total || mediaCards.length;

  const cardsRef = useRef<(HTMLDivElement | null)[]>([]);
  const previousLength = useRef(mediaCards.length);
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (res?.Media?.MediaCard) {
        setMediaCards(res.Media.MediaCard);
        setSearchTotal(res.pagination?.total || res.Media.MediaCard.length);
        setPage(1);
      } else {
        setMediaCards([]);
        setSearchTotal(0);
  ...
  const clearSearch = useCallback(() => {
    ...
    previousLength.current = 0;
    cardsRef.current = [];
    setMediaCards(mediaData?.MediaCard || []);
  ...
  const loadMore = async () => {
    if (loading || loadingMore || mediaCards.length >= total) return;
  ...
  useEffect(() => {
    ...
    previousLength.current = mediaCards.length;
  }, [mediaCards]);

  const getImageUrl = (image: MediaImage[] | null): string => {
            ...
            searchQuery && (
              <div className="w-full flex flex-col items-center justify-center py-12">
                <p className="text-[16px] md:text-[18px] font-normal text-gray-600 text-center">
                  No medias found matching &quot;{searchQuery}&quot;
                </p>
              </div>
            )
          )}
